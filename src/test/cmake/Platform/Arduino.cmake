...
# TEENSY_SPEED_MODE           - CPU Frequency mode for Teensy board
#
...
function(get_arduino_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR BOARD_ID MANUAL)
    ...
    if(BOARD_CORE)
        ...
        # Teensy CPU Speed
        if (TEENSY_SPEED_MODE)
            if (NOT ${BOARD_ID}.menu.speed.${TEENSY_SPEED_MODE}.build.fcpu)
                message(FATAL_ERROR "Teensy CPU Frequency Mode set, but compatible board mode could not be found. [${TEENSY_SPEED_MODE}]")
            endif()
            set(${BOARD_ID}.build.fcpu ${${BOARD_ID}.menu.speed.${TEENSY_SPEED_MODE}.build.fcpu})
        endif()

        # Prepare COMPILE_FLAGS
        set(COMPILE_FLAGS "-DARDUINO_${${BOARD_ID}.build.board} -DARDUINO_ARCH_AVR -DARDUINO=${ARDUINO_VERSION_DEFINE} ${${BOARD_ID}.build.flags.defs} ${${BOARD_ID}.build.flags.cpu} ${${BOARD_ID}.build.flags.dep}")

        # Validate CPU Freq is defined
        if(${BOARD_ID}.build.fcpu)
            set(COMPILE_FLAGS "${COMPILE_FLAGS} -DF_CPU=${${BOARD_ID}.build.fcpu}")
	else()
        ...
        if(NOT MANUAL)
            set(COMPILE_FLAGS "${COMPILE_FLAGS} -I\"${${BOARD_CORE}.path}\" -I\"${ARDUINO_LIBRARIES_PATH}\" -I\"${ARDUINO_LIBRARIES_PATH_DEF}\"")
        endif()

        # Teensy USB Mode
        if (TEENSY_USB_MODE)
            if (NOT ${BOARD_ID}.menu.usb.${TEENSY_USB_MODE}.build.usbtype)
	    ...
	    message(FATAL_ERROR "Teensy USB Mode not set, You have to set TEENSY_USB_MODE")
        endif()

        # Teensy Keyboard Mode
        if (TEENSY_KEYBOARD_MODE)
            if (NOT ${BOARD_ID}.menu.keys.${TEENSY_KEYBOARD_MODE}.build.keylayout)
                message(FATAL_ERROR "Teensy Keyboard Mode set, but compatible board mode could not be found. [${TEENSY_KEYBOARD_MODE}]")
            endif()
            set(COMPILE_FLAGS "${COMPILE_FLAGS} -DLAYOUT_${${BOARD_ID}.menu.keys.${TEENSY_KEYBOARD_MODE}.build.keylayout}")
        else()
	    message(FATAL_ERROR "Teensy Keyboard Mode not set, You have to set TEENSY_KEYBOARD_MODE")
        endif()
        ...
        if(NOT ${_srcfile_generated} OR ${_sketch_generated})
            ...
            foreach(SRC_LINE ${SRC_CONTENTS})
                if("${SRC_LINE}" MATCHES "^[ \t]*#[ \t]*include[ \t]*[<\"]([^>\"]*)[>\"]")
                    ...
                    foreach(LIB_SEARCH_PATH ${LIBRARY_SEARCH_PATH} ${ARDUINO_LIBRARIES_PATH} ${ARDUINO_LIBRARIES_PATH_DEF} ${CMAKE_CURRENT_SOURCE_DIR} ${CMAKE_CURRENT_SOURCE_DIR}/libraries ${ARDUINO_EXTRA_LIBRARIES_PATH})
...
function(SETUP_ARDUINO_EXAMPLE TARGET_NAME LIBRARY_NAME EXAMPLE_NAME OUTPUT_VAR)
    ...
    get_property(LIBRARY_SEARCH_PATH
                 DIRECTORY     # Property Scope
                 PROPERTY LINK_DIRECTORIES)
    foreach(LIB_SEARCH_PATH ${LIBRARY_SEARCH_PATH} ${ARDUINO_LIBRARIES_PATH} ${ARDUINO_LIBRARIES_PATH_DEF} ${CMAKE_CURRENT_SOURCE_DIR} ${CMAKE_CURRENT_SOURCE_DIR}/libraries)
...
if (NOT DEFINED ARDUINO_CXX_FLAGS)
    set(ARDUINO_CXX_FLAGS "${ARDUINO_C_FLAGS} -fno-exceptions -felide-constructors -std=gnu++0x -fno-rtti")
...
#=============================================================================#
#set(ARDUINO_LINKER_FLAGS "-Wl,--gc-sections,--relax,--defsym=__rtc_localtime=1 -larm_cortexM4l_math -lm")
set(ARDUINO_LINKER_FLAGS "-O -Wl,--gc-sections,--relax,--defsym=__rtc_localtime=1 -mthumb -mcpu=cortex-m4 -fsingle-precision-constant -T${ARDUINO_SDK_PATH}/hardware/teensy/avr/cores/teensy3/mk20dx256.ld -larm_cortexM4l_math -lm")
...
if(NOT ARDUINO_FOUND AND ARDUINO_SDK_PATH)
    ...
    find_file(ARDUINO_AVRDUDE_CONFIG_PATH
        NAMES avrdude.conf
        PATHS ${ARDUINO_SDK_PATH} /etc/avrdude /etc ${ARDUINO_SDK_PATH}/avr
